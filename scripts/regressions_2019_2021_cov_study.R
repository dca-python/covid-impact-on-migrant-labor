...
print("Joining with data from SOEP-CoV study…")
# Covid specific
cov_join_20 <- read_dta("./data/raw_data/cov.dta", col_select = c("pcov014a", "pid", "syear", "sample1", "pmonin", "perw_rn", "plb0022_h", "phrf20_ref", "phrf20_core")) %>% inner_join(inner_join_20)

# PARAMETERS
year_pair <- c("19_20_cov") # TODO: Check if it is necessary to make this 19_20 to trigger stuff
inner_join_fy = inner_join_19
empl_in_fy = empl_in_2019
inner_join_sy = cov_join_20
...
sys_mutation <- function(inner_join_sy, empl_in_fy) {
  result <- inner_join_sy %>%
    ...
    # mutate(in_stw = case_when( # this gets added my merging
    #   pgemplst == 7 ~ 1,
    #   pgemplst < 0 ~ NA_integer_,
    #   TRUE ~ 0
    # )) %>%
...
sys <- sys_mutation(inner_join_sy, empl_in_fy) %>%
  mutate(in_iab_sample = case_when(
    sample1 %in% c(30, 31, 34) ~ 1, # in iab sample
    !(sample1 %in% c(30, 31, 34)) ~ 0, # not in iab sample
    TRUE ~ NA_integer_
  )) %>%
  mutate(in_stw = case_when(
    pcov014a == 1 ~ 1, # in stw bc. of covid
    TRUE ~ 0
  )) %>%
  mutate(splitweight = case_when(
    in_iab_sample == 1 ~ phrf20_ref,
    in_iab_sample == 0 ~ phrf20_core,
    TRUE ~ NA_integer_
  )) %>%
  filter(splitweight >= 0)
fys <- fys_mutation(inner_join_fy)

# IF 2020 is SY: SPECIFIC OPERATION
if (year_pair %in% c("19_20")) {
  sys <- sys %>% jan_feb_20_filter() # not necessary for cov.dta, as january/february do not show up and march has 12 obs.
}

# MERGE THE TWO YEARS
workset <- sys %>% left_join(fys, by = "pid")
if (year_pair %in% c("19_20", "18_19")) {  # correctly not triggered if case is 19_20_cov
  workset <- workset %>% select(-in_stw)
}

# CHANGE CATEGORICAL VARIABLES TO FACTORS
categorical_columns <- c(colnames(workset %>% select(-c(hid, pid, syear, immiyear, gebjahr, phrf, pgerwzeit, length_of_stay_years, splitweight))))
...
workset <- workset %>% mutate(splitweight = as.numeric(splitweight))
...
not_employed_general_migrant_grouping <- svyglm(
  ...
  design = workset %>% as_survey_design(weights = splitweight)
)
not_employed_direct_migrant_grouping <- svyglm(
  ...
  family = binomial(link = "logit"),
  design = workset %>% filter(migback == 2) %>% as_survey_design(weights = splitweight)
)
...
create_regression_table_file(
  ...
  table_title = "2019 to 2020, Employed to Unemployed (cov.dta)",
  output_file_name = "employed_to_unemployed_2019_to_2020_cov"
)
create_regression_table_file(
  model_list = list(
  ...
  table_title = "2019 to 2020, Employed to Not-Employed (cov.dta)",
  output_file_name = "employed_to_not_employed_2019_to_2020_cov"
)
if (year_pair %in% c("19_20_cov", "19_21", "20_21", "21_22")) {  # TODO: Check if correct and if yes, generalize to other files.
  create_regression_table_file(
    model_list = list(
    ...
    table_title = "2019 to 2020, Employed to Short-Time Work (cov.dta)",
    output_file_name = "employed_to_short_time_work_2019_to_2020_cov"
  )
}
